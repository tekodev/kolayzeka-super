...
export default function LunaInfluencerShow({ auth }: { auth: any }) {
    const { props } = usePage();
    ...
    }, [(props as any).latest_generation?.status, (props as any).generation?.status]);
    ...
    useEffect(() => {
        ...
        if (framing_type === 'head-to-toe' || framing_type === 'full body') {
            suggestedDistance = 'full-body distance';
             // For full body, we want safe margins to avoid cropping feet/head
            ...
            // For portrait, we want to fill the frame with the face/head
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Luna Influencer Generator</h2>}
        >
            <Head title="Luna Influencer" />
                        ...
                        <div className="lg:col-span-2 bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border border-gray-100">
                           <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <InputLabel value="Kimlik Referans Görselleri (Sabit)" />
                                        <div className="mt-1 p-3 bg-gray-50 border border-gray-200 rounded-md text-sm text-gray-600">
                                            Kimlik referansı <strong>LUNA</strong> modeli olarak sabitlenmiştir. 
                                            Harici fotoğraf yüklemenize gerek yoktur.
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        {showVideoForm && (
                                            <div className="mt-8">
                                                <VideoGenerationForm 
                                                    ...
                                                    routeName="apps.luna-influencer.generate-video"
                                ...
                                {!flash.generation_result && !processing && (
                                    <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-lg p-6 border border-indigo-100 text-center">
                                        ...
                                        <p className="text-indigo-600 text-sm mt-1">
                                            Fill out the form to generate a professional influencer photo powered by Luna.
                                        </p>
                                    </div>
                                )}
                             </div>
